...
import { FormsModule, FormBuilder, FormGroup, Validators, ReactiveFormsModule, FormControl, FormArray } from '@angular/forms';
...
export class CrearCursos implements OnInit {
  ...
  async onConfirmOTP() {
    ...
    this.otpService.verifyOTP(this.pendingId, this.OTPCode).subscribe({
      next: async() => {
        //Aquí, si funciona y no hay error, si creamos el curso (continuamos creación del curso)
  ...
  async onResendOTP(){
    ...
    this.otpService.resendOTP(this.pendingId, this.user?.email || "").subscribe({
      next: () => {
